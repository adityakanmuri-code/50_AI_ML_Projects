...
class CleanData:
    ...
    def features_setting(self,dataframe:pd.DataFrame = None,mappings = None):
        try:
            logging.info("______________________________________________________")
            logging.info('Encoding the Target Data while using a range of values')
            logging.info("______________________________________________________")
            for col,config in mappings.items():
                ...
                if operator == '>=':
                    dataframe[col] = dataframe[col].apply(
                        lambda x : positive_value if x >= threshold else negative_value if pd.notna(x) else x
                    )
                elif operator == '>':
                    dataframe[col] = dataframe[col].apply(
                        lambda x : positive_value if x > threshold else negative_value if pd.notna(x) else x
                    ...
                    logging.info(f'{operator} is not a valid operator')
                    raise ValueError(f'{operator} is not a valid operator')
            logging.info("_________________________________________________________________________")
            logging.info('Encoding the Target Data while using a range of values has been completed')
            logging.info("_________________________________________________________________________")
            return dataframe
        except Exception as e:
            raise CustomException(e,sys)
